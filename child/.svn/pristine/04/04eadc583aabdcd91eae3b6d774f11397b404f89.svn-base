...
angular.module('App').controller('MessageSetupCtrl', function($rootScope, $stateParams,$scope, $ajax,$uibModal) {
    $scope.currentTab = 'message.setup';
    ...
    if(remind==undefined){
        remind= sessionStorage.getItem("remind2");
    }
    ...
    $scope.load = function() {
        //数据推送、
        // $ajax.post('/api-child/ordinary/DataPush/detail', $.param({username:username}), function(result) {
        //     // console.log(result)
        //     if( result != null){
        //         if(result.type==1){
        ...
        //
        // })
        $ajax.post('/api-child/ordinary/Whitelist/status', $.param({username:username}), function(result) {
            // console.log(result);
            if(result.is_stop==0){
    ...
    $scope.modify = function() {
        ...
        $('.form-horizontal').validate({
            ...
            submitHandler: function() {
                ...
                var url ='/api-child/ordinary/Whitelist/edit';
    ...
    $scope.loadDuanxing=function () {
        ...
        $ajax.post('/api-child/ordinary/Account/remind', $.param({username:username,remind:remind}), function(result) {
            layer.msg('设置成功!', {time: 1000, icon: 1});
        })
    }
    ...
    $scope.Datapush2=function () {
        $ajax.post('/api-child/ordinary/DataPush/edit', $.param({username:username,type:1,status_report:'',
    ...
    $scope.Datapush1=function () {
        $ajax.post('/api-child/ordinary/DataPush/edit', $.param({
            username:username,
            type:2,
...
angular.module('App').controller('MessagePlatBlackEditCtrl', function($rootScope, $stateParams, $scope, $ajax, $uibModalInstance, data) {
    ...
    var s = '新增';
    var url = '/api-child/ordinary/Blacklist/add';
    if ($scope.data) {
        $scope.data = {
            id: $scope.data.id,
