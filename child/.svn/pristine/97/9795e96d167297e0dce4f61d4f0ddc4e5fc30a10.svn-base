...
angular.module('App').controller('MessageBillsCtrl', function($rootScope, $stateParams, $scope, $ajax ,$filter) {
    ...
    $scope.currentTab = 'message.bills';

    $scope.isActiveTab = function(url) {
        return url == $scope.currentTab;
    };

    $scope.types = [{name: '全部', value: ''}, {name: '充值', value: 1}, {name: '注册赠送', value: 3},{name: '体验赠送', value: 4}];
    ...
    $scope.load = function() {
        ...
        $ajax.post('/api-child/ordinary/Bill/items/',$.param(data),function (result) {
