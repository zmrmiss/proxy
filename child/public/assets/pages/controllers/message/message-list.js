...
angular.module('App').controller('MessageSignCtrl', function($rootScope, $stateParams, $scope, $ajax, $uibModal) {
    ...
    $scope.load = function() {
        ...
        $ajax.post('/api-child/ordinary/Signature/items',$.param(data),function (result) {
            // alert(result);
    ...
    $scope.DelSign = function(id) {
        ...
        layer.msg('你确定删除吗？', {
            ...
            yes: function(index){
                layer.close(index);
                $ajax.post('/api-child/ordinary/Signature/remove', $.param({
                    id:id
                }), function (result) {
...
angular.module('App').controller('MessageSignEditCtrl', function($rootScope, $stateParams, $scope, $ajax, $uibModalInstance, data) {
    ...
    var url = '/api-child/ordinary/Signature/add';
    $scope.ok = function () {
        ...
        $('#signForm').validate({
            ...
            submitHandler: function() {
                var datas={username: $rootScope.username2||sessionStorage.getItem("qrname2"), content: $scope.data.content1, remark: $scope.data.remark};
