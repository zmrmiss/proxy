 ...
 *
 */
angular.module('App').controller('MessageIndustrySendRoutineCtrl', function($rootScope, $stateParams, $uibModal, $scope, $ajax, FileUploader ,$filter) {
    $scope.HtmlRen = true;
    ...
    //通讯录加入的手机号
    $rootScope.MaillistMobiles2=0;

    // 检测获得的手机号
    $rootScope.batcheckMpbiles2=0;

    //上传+缓存手机号
    $rootScope.batupLoadMpbiles2=0;
    ...
    $scope.checkMobiles = function() {
        ...
        $ajax.post('/api/marketing/Write/check', $.param({
            ...
            $rootScope.batcheckMpbiles2 = result.valid;
        });
    };
    ...
    $scope.formcookie = function () {
        $ajax.post('/api/marketing/Ignore2/load/',$.param({username:apindustry}),function (result) {
            if(result!=null){
                $scope.showResult=true;
                $scope.shower=result;
                $rootScope.batupLoadMpbiles2=result.valid;
            }
        })
    }
    $scope.formcookie();
    ...
    $scope.clerrform = function () {
        layer.msg("你确认清空号码缓存吗？", {
            ...
            yes: function(index){
                layer.close(index);
                $ajax.post('/api/marketing/Ignore2/clean/',$.param({username: apindustry}) , function () {
                    $scope.showResult=false;
                    $scope.batchshow=false;
                    $('#uploadpress1').html('');
                    $rootScope.batupLoadMpbiles2=0;
    ...
    $scope.opencheckedit = function() {
        $ajax.post('/api/marketing/Ignore/check', $.param({username:apindustry}),
            function(result) {
                if(result.list!=null){
                    $scope.sname='single';
                    $uibModal.open({
    ...
    $scope.opencheckMail = function() {
        $ajax.post('/api/marketing/IgnoreBooks/check', $.param({username:apindustry}),
            function(result) {
                if(result.list!=null){
        ...
        $ajax.post('/api/marketing/Handle/check',
            ...
            function(result) {
                ...
                if($scope.batch.valid>0){
                    $rootScope.batupLoadMpbiles2=$scope.batch.valid;
                    $scope.telDelete();
    ...
    $scope.Summary = function () {
        // $scope.batMobiles2=parseInt($rootScope.batcheckMpbiles2)+parseInt($rootScope.batupLoadMpbiles2)+parseInt($rootScope.MaillistMobiles2);
    ...
    $scope.SummarySub2 = function () {
        ...
        $ajax.post('/api/marketing/Duplicate/remove',$.param({username:apindustry}),function (result) {
            $scope.batMobiles2=result.valid;
            //计费条数
            $scope.countForm2=$scope.batMobiles2*$scope.count;
            $uibModal.open({
                backdrop: 'static',
                templateUrl: 'sendMsgSummary.html',
                controller: 'IndustrySummaryCtrl',
                size: 'md',
                resolve:{
                    data : function(){
                        return {
                            mobiles:angular.copy($scope.batMobiles2),
                            counts:angular.copy($scope.countForm2),
    ...
    $scope.openCheckFilteratet = function () {
        var regex = /^1([356789]\d|4[57])\d{8}$/;
        var arr=[];
        for (var k in $rootScope.routmobiles2){
            if(!regex.test($rootScope.routmobiles2[k])){
                // return;
            }else {
                arr.push($rootScope.routmobiles2[k]);
            }
        }
    ...
    //    格式化号码框
    $scope.FormatTel = function () {
        if($rootScope.data.mobiles.match(/((((1[3,4,5,6,7,8,9]))\d{9}))/g)==null){
            $('#mobileschange').val('');
            $rootScope.data.mobiles='';
        }else {
            $('#mobileschange').val($rootScope.data.mobiles.match(/((((1[3,4,5,6,7,8,9]))\d{9}))/g).join('\n'));
            $rootScope.data.mobiles=$rootScope.data.mobiles.match(/((((1[3,4,5,6,7,8,9]))\d{9}))/g).join('\n');
        }
        $scope.checkMobiles();
    }
                    ...
                    function gex1() {
                        ...
                        function CheckList() {
                            ...
                            $ajax.post('/api/marketing/Books/check', $.param({
                                ...
                                $scope.Maillist = result;
                                // 带加入手机号总和
                                // $rootScope.MaillistMobiles2 = result.valid;
                            });
                            $scope.arr6s=[]
                        }
...
//短信发送前弹框
angular.module('App').controller('IndustrySummaryCtrl', function($rootScope, $stateParams, $uibModal,$scope, $ajax, $uibModalInstance, data) {
    $scope.sendData=data;
    ...
    $scope.getId = function () {
        ...
        if(data.ids){
            if($scope.option=='single'){
                telEdit = data.mobile.split(",");

                //做比较的两个数组
                var array1 = $rootScope.routmobiles2;//数组1
...
//临时数组存放
                ...
                $ajax.post('/api/marketing/Ignore/remove', $.param({
                    ids: data.ids,
                    username:apindustry}), function(result) {
                    $rootScope.batcheckMpbiles2=$rootScope.batcheckMpbiles2-array2.length;
                ...
                $ajax.post('/api/marketing/Ignore2/remove', $.param({
                    ids: data.ids,
                    username:apindustry}), function(result) {
                    $rootScope.batupLoadMpbiles2=$rootScope.batupLoadMpbiles2-data.ids.split(",").length;
                    layer.msg("操作成功", {time: 2000, icon: 1});
                });
                ...
                $ajax.post('/api/marketing/IgnoreBooks/remove', $.param({
                    ids: data.ids,
                    username:apindustry}), function(result) {
                    // $rootScope.MaillistMobiles2=$rootScope.MaillistMobiles2-data.ids.split(",").length;
                    layer.msg("操作成功", {time: 2000, icon: 1});
                });
            }
